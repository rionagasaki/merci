        ...
        VStack {
            VStack(alignment: .leading){
                ...
                if let chatPairLastMessage = pairModel.pair.chatPairLastMessage[pair.pair.id] {
                    HStack {
                        if userModel.chatUnreadNum[pairModel.pair.chatPairIDs[pair.pair.id]!] != 0
                        {
                            Circle()
                                .foregroundColor(.red)
                        ...
                        Text(chatPairLastMessage)
                            ...
                            .font(.system(size:16,weight: userModel.chatUnreadNum[pairModel.pair.chatPairIDs[pair.pair.id]!] != 0
                                          ? .bold: .light))
                    }
                }
